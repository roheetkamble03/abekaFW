...
public @interface DataBaseQueryConstant {
    ...
    String MY_TO_DO_LIST_AD_DB = "SELECT assign.student_id, assign.apref, assign.assignmentid, coalesce(assign.da_status, 0) AS da_status,\n" +
            "                coalesce(assign.pr_form, 0) AS pr_form, coalesce(assign.pr_version, ' ') AS pr_version, coalesce(assign.pr_school, ' ') AS pr_school, coalesce(assign.pr_boxletter, ' ') AS pr_boxletter,\n" +
            "                coalesce(assign.pr_itemnumber, 0) AS pr_itemnumber, coalesce(assign.pr_subitemnumber, 0) AS pr_subitemnumber,\n" +
            "                coalesce(a.subject, ' ') AS subject, a.short_description, a.long_description,\n" +
            "                0 AS testid,\n" +
            "                to_char(start_date, 'YYYY-MM-DD') AS start_date,\n" +
            "                'T' || to_char(start_date, 'hh24:mi:ss') AS start_time,\n" +
            "                CASE WHEN assign.all_day = 'Y' THEN to_char(end_date + 1, 'YYYY-MM-DD') ELSE to_char(end_date, 'YYYY-MM-DD') END AS end_date,\n" +
            "                'T' || to_char(end_date, 'hh24:mi:ss') AS end_time, assign.all_day, c.description AS category,\n" +
            "                c.categoryid,  coalesce(c.color,' ') AS color, a.lesson_number, a.lesson_number AS segmentlesson, 'Y' AS editable, completion_date, 'STUDENTNORMAL ' || coalesce(typ.name, '') AS type_name, student_assignments_id AS eventid,\n" +
            "                assign.subscription_items_fk, assign.subject_id_fk, assign.segment_id_fk, lock_assignment, 3 AS sort_order\n" +
            "           FROM homeschoolhouse.student_assignments assign\n" +
            ...
            "          WHERE assign.student_id = 'STUDENT_ID'\n" +
            "            AND start_date between to_date('START_DATE', 'YYYYMMDD') and to_date('END_DATE'||'235959', 'YYYYMMDDhh24miss')\n" +
            "            AND c.categoryid != 3\n" +
            "          UNION\n" +
            "         SELECT to_number('STUDENT_ID') AS student_id, 0 AS apref, -1 AS assignmentid, 0 AS da_status,\n" +
            "                0 AS pr_form, ' ' AS pr_version, ' ' AS pr_school, ' ' AS pr_boxletter,\n" +
            "                0 AS pr_itemnumber, 0 AS pr_subitemnumber,\n" +
            "                ' ' AS subject, (SELECT Utilities.Format.ProperName(holiday) FROM dual) AS short_description, 'Enjoy the ' || (SELECT Utilities.Format.ProperName(holiday) FROM dual) || ' holiday!' AS long_description,\n" +
            "                0 AS testid,\n" +
            ...
            "                c.categoryid,  coalesce(c.color,' ') AS color, 0 AS lesson_number, 0 AS segmentlesson, 'N' AS editable, null AS completion_date, 'HOLIDAY' AS type_name, 0 AS eventid,\n" +
            "                0 AS subscription_items_fk, 0 AS subject_id_fk, 0 AS segment_id_fk, ' ' AS lock_assignment, 4 AS sort_order\n" +
            "           FROM linc.abadb_holid\n" +
            "           JOIN homeschoolhouse.category c\n" +
            "             ON accountnumber in (ACCOUNT_NUMBER, 1)\n" +
            "          WHERE description = 'Holiday'\n" +
            "            AND to_date(ho_beg_dt, 'YYYYMMDD') between to_date('START_DATE', 'YYYYMMDD') and to_date('END_DATE'||'235959', 'YYYYMMDDhh24miss')\n" +
            "          UNION\n" +
            "         SELECT assign.student_id, assign.apref, coalesce(assign.assignmentid,0), coalesce(assign.da_status, 0) AS da_status,\n" +
            "                coalesce(assign.pr_form, 0) AS pr_form, assign.pr_version, assign.pr_school, assign.pr_boxletter,\n" +
            "                assign.pr_itemnumber, coalesce(assign.pr_subitemnumber, 0) AS pr_subitemnumber, assign.subject,\n" +
            "                CASE WHEN tnp.eno_grade IN (1,2,3,4,5,6,14,15) THEN tsubj.description ELSE tnp.eno_desc END AS short_description, replace(tboxd.txd_desc, '\"\"', '\\\"\"') || ' (Day ' || assign.lesson_number || ')' AS long_description,\n" +
            ...
            "                c.categoryid,  coalesce(c.color,' ') AS color, assign.lesson_number, assign.lesson_number AS segmentlesson, 'N' AS editable, completion_date, 'TBOXITEMS' AS type_name, student_assignments_id AS eventid,\n" +
            "                assign.subscription_items_fk, assign.subject_id_fk, assign.segment_id_fk, lock_assignment, 2 AS sort_order\n" +
            "           FROM homeschoolhouse.student_assignments assign\n" +
            "           JOIN homeschoolhouse.category c\n" +
            "             ON accountnumber = 1\n" +
            ...
            "          UNION\n" +
            "         SELECT assign.student_id, assign.apref, coalesce(assign.assignmentid,0) AS assignmentid, coalesce(assign.da_status, 0) AS da_status,\n" +
            "                coalesce(assign.pr_form, 0) AS pr_form, assign.pr_version, assign.pr_school, assign.pr_boxletter,\n" +
            "                assign.pr_itemnumber, assign.pr_subitemnumber, assign.subject,\n" +
            "                CASE WHEN tnp.eno_grade IN (1,2,3,4,5,6,14,15) THEN tsubj.description ELSE tnp.eno_desc END AS short_description, replace(tboxm.txm_desc, '\"\"', '\\\"\"') || ' (Day ' || tboxm.txm_lesson || ')' AS long_description,\n" +
            ...
            "                 'Video Lesson ' || segments.lesson || CASE WHEN TRIM(subj.subjectname) = TRIM(stype.sessionname) THEN '' ELSE ' - ' || coalesce(stype.sessionname, '') END AS long_description,\n" +
            "                null AS testid,\n" +
            "                to_char(assign.start_date, 'YYYY-MM-DD') AS start_date,\n" +
            "                'T' || to_char(assign.start_date, 'hh24:mi:ss') AS start_time,\n" +
            "                CASE WHEN coalesce(assign.all_day,'Y') = 'Y' THEN to_char(assign.end_date + 1, 'YYYY-MM-DD') ELSE to_char(assign.end_date, 'YYYY-MM-DD') END AS end_date,\n" +
            "                'T' || to_char(assign.end_date, 'hh24:mi:ss') AS end_time, coalesce(assign.all_day,'Y') AS all_day, c.description AS category,\n" +
            "                c.categoryid,  coalesce(c.color,' ') AS color, assign.lesson_number, segments.lesson AS segmentlesson, 'N' AS editable, completion_date, typ.name AS type_name, student_assignments_id AS eventid,\n" +
            "                assign.subscription_items_fk, assign.subject_id_fk, assign.segment_id_fk, lock_assignment, 1 AS sort_order\n" +
            "           FROM homeschoolhouse.student_assignments assign\n" +
            "           JOIN linc.abadb_appld ap\n" +
            "             ON assign.apref = ap.ap_apref\n" +
            ...
            "            AND assign.start_date between to_date('START_DATE', 'YYYYMMDD') and to_date('END_DATE'||'235959', 'YYYYMMDDhh24miss')\n" +
            "            AND ap.ap_status != 9\n" +
            "            AND (rchdr.rch_status IS NULL OR rchdr.rch_status != 'C')\n" +
            ...
            "                c.description AS Category,\n" +
            "                c.categoryid, coalesce(c.color,' ') AS color, assign.lesson_number, assign.lesson_number AS segmentlesson, 'N' AS editable, COMPLETION_DATE, 'PE' AS type_name, student_assignments_id AS eventid,\n" +
            "                assign.subscription_items_fk, assign.subject_id_fk, assign.segment_id_fk, lock_assignment, 3 AS sort_order\n" +
            "           FROM homeschoolhouse.student_assignments assign\n" +
            "           JOIN linc.abadb_appld ap\n" +
            "             ON assign.apref = ap.ap_apref\n" +
            ...
            "            AND ap.ap_status != 9\n" +
            "            AND (rchdr.rch_status IS NULL OR rchdr.rch_status != 'C')\n" +
            "ORDER BY start_date ASC, short_description ASC, long_description ASC";
